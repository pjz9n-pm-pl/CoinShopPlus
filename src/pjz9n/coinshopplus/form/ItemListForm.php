...
class ItemListForm extends AbstractMenuForm
{
    /** @var Folder */
    private $folder;

    public function __construct(Folder $folder, Player $player)
    {
        $this->folder = $folder;
        $options = [];
        $options[] = [
        ...
        parent::__construct(
            Language::get()->translateString("shop") . " - " . Language::get()->translateString("shop.item.select"),
    ...
    public function onSubmit(Player $player, int $selectedOption): void
    {
        ...
        //アイテムが選択された
        if (count($shopItem->getAcceptTypes()) === 1) {
            //種別がひとつだった場合
            $player->sendForm(new SetCountForm($shopItem->getAcceptTypes()[0], $shopItem, $player));
        } else {
            //種別が複数あった場合
            $player->sendForm(new SetTypeForm($shopItem, $player));
        }
    }
}
